...
abstract class CarWidgetRepository {
  ...
  Future<CarWidgetState> extractPathAndName(BuildContext context , String svgPath);
...
class CarWidgetRepo extends CarWidgetRepository {
  ...
  @override
  Future<CarWidgetState> extractPathAndName(BuildContext context , String svgPath) async {
  ...
  List buildSvgImageList(List<Path> imagePathList , List imagePathName){
    List clickAbleList = ['id="eksedam-LF"','id="rfrf-LB"','id="door-LB"','id="door-LF"','id="light-spot-LB"','id="wheel-LF"','id="glass-LB"','id="shoraa-LB"','id="rfrf-LF"','id="atbl"','id="eksedam-LB"','id="light-spot-LF"'];
    List<CarModel> svgImageList = [] ;
    for(int i = 0 ; i < imagePathList.length ; i ++ ){

      svgImageList.add(CarModel(PathModel(imagePathList[i] , imagePathName[i]) , Colors.white  , !clickAbleList.contains(imagePathName[i]))) ;
